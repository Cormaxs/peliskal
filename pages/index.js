...
import { useState, useEffect, useMemo, useCallback } from 'react'; // Agregué useCallback
...
export default function Home({ contentData, currentQuery }) {
    ...
    const moviesDB = useMemo(() => contentData?.movies || [], [contentData]);

    const [randomCatalog, setRandomCatalog] = useState(moviesDB);

    useEffect(() => {
        // Optimización: Solo mezclar si hay datos y evitar re-ejecuciones innecesarias
        if (moviesDB.length > 0) {
            const shuffled = [...moviesDB].sort(() => 0.5 - Math.random());
            setRandomCatalog(shuffled);
        }
    }, [moviesDB]);
    ...
    // OPTIMIZACIÓN: Función de filtrado memorizada para evitar re-cálculos
    const filterByGenre = useCallback((id, list) => {
        return list.filter(m => m.genres?.includes(id)).slice(0, 20);
    }, []);

    const recentlyAdded = useMemo(() => [...moviesDB].reverse().slice(0, 20), [moviesDB]);

    // OPTIMIZACIÓN: Secciones memorizadas con dependencias estrictas
    const sections = useMemo(() => {
        if (!randomCatalog.length) return [];
        return [
            ...
            { title: "Acción Trepidante", data: filterByGenre(GENRES.action, randomCatalog) },
            { title: "Ciencia Ficción y Futuro", data: filterByGenre(GENRES.scifi, randomCatalog) },
            { title: "Joyas Ocultas", data: randomCatalog.slice(40, 60) }, 
            { title: "Terror y Suspenso", data: filterByGenre(GENRES.horror, randomCatalog) },
            { title: "Grandes Historias (Drama)", data: filterByGenre(GENRES.drama, randomCatalog) },
            { title: "Comedia para reír", data: filterByGenre(GENRES.comedy, randomCatalog) },
            { title: "Documentales", data: filterByGenre(GENRES.documentary, randomCatalog) },
            { title: "Animación", data: filterByGenre(GENRES.animation, randomCatalog) },
        ];
    }, [randomCatalog, recentlyAdded, filterByGenre, GENRES]);
        ...
        <div className="peliskal-theme">
            ...
            <main className="content-rows">
                {sections.map((sec, idx) => (
                    sec.data.length > 0 && (
                        <section className="row" key={sec.title}>
                                            ...
                                            <a className="card" onClick={() => setActiveItem(item)}>
                                                ...
                                                <div className="card-image-wrapper">
                                                    <Image 
                                                        ...
                                                        priority={idx < 1 && itemIdx < 5} // Más agresivo con la prioridad inicial
                                                        unoptimized
                                                    />
                                                </div>
            ...
            <style jsx global>{`
                ...
                .hero { height: 85vh; display: flex; align-items: center; padding: 0 5%; background-size: cover; background-position: center; transition: background-image 0.8s ease-in-out; position: relative; will-change: background-image; }
                ...
                .card { flex: 0 0 auto; width: 180px; aspect-ratio: 2/3; border-radius: 12px; position: relative; overflow: hidden; transition: 0.4s; will-change: transform; }
...
export async function getServerSideProps({ query, res }) {
    // OPTIMIZACIÓN DE CACHÉ: Esto hace que volver atrás o navegar al inicio sea instantáneo
    res.setHeader(
        'Cache-Control',
